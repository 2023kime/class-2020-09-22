...
- the difference between new and old u_grad (course enrollment) numbers

After we have a model, Temperance suggests we should:
- recognize the limits of our models
- think about how the model might not reflect the real reasons students are dropping
...
```{r Scene 3, include=TRUE}

the_data<- inner_join(sep_old, sep_new, by=c("id", "title", "name", "department"))
decreases<- the_data%>%
...
increases<- the_data%>%
  ...
  head(n=5)
increases
changes<- full_join(decreases, increases)
changes
ggplot(changes, aes(name, change))+
  geom_col()
